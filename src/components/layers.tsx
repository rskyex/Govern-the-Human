...
/* ── ghost-shell points — outline of the human subject ── */
...
function LayerCanvas({ layerIndex }: { layerIndex: number }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      drawGlassRibbon(ctx, w, h, t, {
        x0: 0.68, y0: 0.28 + depth * 0.05,
        x1: 0.88, y1: 0.68 + depth * 0.05,
      ...
      drawMembrane(ctx, w, h, t, {
        points: ghostShellPoints(0.78, 0.5, 1.2 + depth * 0.4),
        opacity: (0.15 + depth * 0.05) * opMul, hue: 'silver',
        phaseOffset: 5.0 + depth * 2,
      })
      ...
      if (depth === 0) {
        ...
        ctx.fillStyle = hg
        ctx.fill()
        ctx.strokeStyle = `rgba(255,255,255,${0.06 * opMul})`
        ctx.lineWidth = 0.4
        ctx.stroke()

        headPath(ctx, s * 0.75)
        ctx.strokeStyle = `rgba(255,255,255,${0.04 * opMul})`
        ctx.lineWidth = 0.4
        ctx.stroke()
      } else if (depth === 1) {
        ...
        ctx.strokeStyle = `rgba(255,255,255,${0.02 * opMul})`
        ctx.lineWidth = 0.4
        ctx.stroke()
        ctx.restore()
        ctx.restore()
        ...
        ctx.fill()
        ctx.strokeStyle = `rgba(255,255,255,${0.05 * opMul})`
        ctx.lineWidth = 0.4
        ctx.stroke()
        ctx.restore()
      } else {
        ...
        ctx.fillStyle = tg
        ctx.fill()
        ctx.strokeStyle = `rgba(255,255,255,${0.025 * opMul})`
        ctx.lineWidth = 0.4
        ctx.stroke()
        ctx.restore()
        ...
        ctx.fill()
        ctx.strokeStyle = `rgba(255,255,255,${0.03 * opMul})`
        ctx.lineWidth = 0.4
        ctx.stroke()
